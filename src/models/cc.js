...
class CC {
   ...
   */
  static async getAllCCs(options = {}) {
    const req = pool
      .request()
    ...
    return result.recordset.map((cc) => {
      ...
      if (options?.indicateAdmin !== undefined)
        output.isAdmin = cc.IsAdmin === 1;
      return output;
    });
  }
   ...
   * @param {{name?: string, location?: {lat: number, lon: number}}} fields
   */
  static async updateCC(ccId, fields) {
    ...
    if (location)
      setParts.push("Location = geography::Point(@lat, @lon, 4326)");
   ...
   */
  static async isAdmin(userId, ccId) {
    const result = await pool
      .request()
      .input("userId", userId)
      .input("ccId", ccId)
   ...
   */
  static async getAdminByPhoneNumber(ccId, phoneNumber) {
    /** @type {sql.IResult<{UserId: number, Name: string, PhoneNumber: string, Bio: string, ProfilePhotoURL: string}>} */
    const result = await pool
      .request()
      .input("ccId", ccId)
   ...
   */
  static async makeAdmin(ccId, userId) {
    const result = await pool
      .request()
      .input("ccId", ccId)
      .input("userId", userId)
   ...
   * @return {Promise<boolean>}
   */
  static async removeAdmin(ccId, userId) {
    const result = await pool
      .request()
      .input("ccId", ccId)
      .input("userId", userId)
